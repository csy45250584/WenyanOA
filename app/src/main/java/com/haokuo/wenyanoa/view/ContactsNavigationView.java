...
import com.haokuo.wenyanoa.util.utilscode.ConvertUtils;

/**
 * Created by zjf on 2018-08-06.
 */

public class ContactsNavigationView extends View {
    /*绘制的列表导航字母*/
    ...
    public ContactsNavigationView(Context context) {
        super(context);
    }

    public ContactsNavigationView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        bgPaint = new Paint();
        bgPaint.setAntiAlias(true);
        bgPaint.setColor(ContextCompat.getColor(context, R.color.colorPrimary));
        wordsPaint = new Paint();
        wordsPaint.setAntiAlias(true);
        wordsPaint.setColor(Color.GRAY);
        wordsPaint.setTextSize(getResources().getDimension(R.dimen.sp_16));
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //使得边距好看一些
        int height = getMeasuredHeight() - ConvertUtils.dp2px(getResources().getDimension(R.dimen.dp_4));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            float wordY = itemWidth / 2 + i * itemHeight + ConvertUtils.dp2px(getResources().getDimension(R.dimen.dp_2));
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                setBackgroundColor(ContextCompat.getColor(mContext, R.color.textLightScrims));
